    ...
    if($con) {
        ...
        $id = $_GET["id"];
        showMedicineRelatedData($id);
        }
    }

    
    function createMedicineStock() {
        require "db_connection.php";
        $name = $_GET['name_drug'];
        $batch_id = $_GET['batch_id'];
        $raw_date = $_GET['exp_date'];
        $date_arr = explode('-',$raw_date);
        $exp_date = $date_arr[1].'/'.$date_arr[0];
        $mrp = $_GET['mrp'];
        $rate = $_GET['rate'];
        $qty = $_GET['qty'];
        if($con) {
            $query = "INSERT INTO medicines_stock (NAME,BATCH_ID, EXPIRY_DATE, QUANTITY, MRP, RATE) VALUES('$name', $batch_id, '$exp_date', $qty, $mrp, $rate)";
            $result = mysqli_query($con, $query);
            if(!empty($result))
                echo "$name! stock added...";
            else
                echo "Failed to add $name! to stock";
        }
    }

    function showMedicineRelatedData($id) {
        require "db_connection.php";
        if($con) {
