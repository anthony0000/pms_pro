...
function fillFields(medicine_name, id) {
    ...
    fill(medicine_name, 'expiry_date_' + id, 'EXPIRY_DATE','ONLYTEXT');
...
function fill(name, field_name, column,datatype) {
    console.log(datatype);
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function () {
        if (xhttp.readyState = 4 && xhttp.status == 200){
            if(datatype == "ONLYTEXT")
                document.getElementById(field_name).value = xhttp.responseText.trim();
            else
                document.getElementById(field_name).value = parseInt(xhttp.responseText);
        }
    };
